...
def balanced(m):
    ...
    if is_mobile(m):
        ...
        same_side_torques = left_length * \
            total_weight(end(left_side)) == right_length * \
            total_weight(end(right_side))
        same_sub_torques = balanced(
            end(left_side)) == balanced(end(right_side))
        return same_side_torques and same_sub_torques
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        return tree(label(t), [replace_leaf(b, old, new) for b in tree_branches])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        nonlocal balance
        nonlocal used_password_list
        nonlocal password

        if len(used_password_list) == 3:
            return "Your account is locked. Attempts: [" + "'" + used_password_list[0] + "', " + "'" + used_password_list[1] + "', " + "'" + used_password_list[2] + "']"
        if input_password == password:
            ...
            if (len(used_password_list) < 3):
                used_password_list.append(input_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    init_status = withdraw(0, old_password)
    if type(init_status) == str:
        return init_status
