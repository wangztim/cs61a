...
def replace_leaf(t, old, new):
    ...
    if (is_leaf(t) and label(t) == old):
        ...
        bs = [replace_leaf(branch, old, new) for branch in branches(t)]
...
def balanced(m):
    ...
    if (is_side(m)):
        ...
        left_side = left(m)
        right_side = right(m)
        if (get_torque(left_side) != get_torque(right_side)):
            return False
        else:
            leftpo = balanced(left_side)
            rightpo = balanced(right_side)
